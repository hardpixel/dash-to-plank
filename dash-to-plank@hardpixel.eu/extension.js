...
import { PlankTheme } from './theme.js'
import { AppsLauncher } from './launcher.js'
...
class DashToPlank extends GObject.Object {
  ...
  _onPlankStarted() {
    this.plankConf  = Utils.getPlankSettings(DOCK_ID)
  ...
  activate() {
    GLib.idle_add(GLib.PRIORITY_LOW, () => {
      try {
        ...
        Main.notifyError('Dash to Plank', 'Plank is not available on your system.')
      }
    })
  }
...
export default class Extension extends Ext.Extension {
  enable() {
    this.dashToPlank = new DashToPlank(this)
    this.dashToPlank.activate()
  }

  disable() {
    this.dashToPlank.destroy()
    this.dashToPlank = null
  }
}
